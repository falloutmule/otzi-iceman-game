...
.aim-strip { position:absolute; right:10px; top:12px; width:42px; height:86px; display:grid; place-items:center; border:1px solid rgba(143,192,169,.45); color:var(--green); background:rgba(12,22,18,.58); writing-mode:vertical-rl; font-weight:900; pointer-events:auto; touch-action:none; }
.action-cluster { position:absolute; right:10px; bottom:max(12px,env(safe-area-inset-bottom)); display:grid; grid-template-columns:80px 80px; gap:8px; pointer-events:auto; }
...
@media (min-width: 720px) { .controls { min-height:188px; } .stick-zone { left:calc(50% - 250px); } .action-cluster { right:calc(50% - 250px); } .aim-strip { right:calc(50% - 250px); } }
`;
